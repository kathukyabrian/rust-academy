...
- the borrower checker compares scopes to determine whether all borrows are valid
- does this at compile time
- to explain the previous example x has a shorter lifetime than r
...
```
- here, we want the function to take string slices which are references rather than the strings because we don't want the longest function to take ownership of its parameters
...
error: could not compile `chapter10` (bin "chapter10") due to 1 previous error
```
- this error reveals that the return type needs a generic lifetime parameter on it because Rust can't tell whether the reference being returned refers to x or y - the if block returns a reference to x and the else block returns a reference to y
